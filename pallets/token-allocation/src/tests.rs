...
use frame::{
	testing_prelude::*,
	token::fungible::{InspectHold, Mutate},
};
...
fn full_lifecycle_works() {
	...
	new_test_ext(vec![], vec![]).execute_with(|| {
		...
		assert_eq!(Balances::free_balance(ben), 100, "Upfront should be free immediately");
		assert_eq!(Balances::total_balance_on_hold(&ben), 900, "Remaining should be held");
		...
		run_to_block(120);
		assert_eq!(Balances::free_balance(ben), 1000, "All funds should be free");
...
fn pagination_logic_handles_overflowing_allocations() {
	...
	new_test_ext(vec![], vec![]).execute_with(|| {
		...
		assert!(crate::PayoutCursor::<Test>::get().is_some(), "Cursor should be set");
		// Epoch should NOT have changed yet.
		assert_eq!(crate::EpochIndex::<Test>::get(), 0, "Epoch should not increment yet");

		// --- EXECUTION BLOCK 101 ---
		run_to_block(101);

		// CHECK 2: Processing finished.
		assert_eq!(crate::Allocations::<Test>::iter_keys().count(), 0, "All allocations processed");
		assert!(crate::PayoutCursor::<Test>::get().is_none(), "Cursor cleared");
...
fn math_is_safe_with_u256() {
	...
	new_test_ext(vec![], vec![]).execute_with(|| {
		...
		assert_eq!(Balances::total_balance_on_hold(&ben), huge_cap, "Funds should be held");
