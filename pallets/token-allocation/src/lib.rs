...
use frame::{
	deps::sp_core::U256,
	prelude::*,
	token::fungible::{Inspect, Mutate, MutateHold},
};
...
type InitialAllocation<T> =
	(EnvelopeId, <T as frame_system::Config>::AccountId, BalanceOf<T>, Option<BlockNumberFor<T>>);
...
pub mod pallet {
	...
	pub struct GenesisConfig<T: Config> {
		pub envelopes: Vec<(EnvelopeId, EnvConfigOf<T>)>,
		pub initial_allocations: Vec<InitialAllocation<T>>,
	}

	impl<T: Config> Default for GenesisConfig<T> {
		fn default() -> Self {
			Self { envelopes: Vec::new(), initial_allocations: Vec::new() }
		}
	}
	...
	impl<T: Config> BuildGenesisConfig for GenesisConfig<T> {
		fn build(&self) {
			for (id, cfg_in) in &self.envelopes {
				assert!(!Envelopes::<T>::contains_key(id), "duplicate envelope in genesis");
	...
	pub enum Event<T: Config> {
		...
		EpochPayout { epoch: u64, at: BlockNumberFor<T>, cursor: Option<AllocationId> },
	}
	...
	impl<T: Config> Pallet<T> {
		pub(crate) fn do_add_allocation(
			...
			ensure!(new_distributed <= cfg.total_cap, Error::<T>::EnvelopeCapExceeded);
