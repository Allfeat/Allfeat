...
//! Integrity tests of the mainnet tokenomics genesis.

use crate::{tests::new_test_ext, *};
use allfeat_runtime_common::currency::UNIT as AFT;
...
fn treasury_allocations_are_correctly_locked() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(free_treasury, expected_upfront, "Treasury liquid balance (upfront) mismatch");
		...
		assert_eq!(held_treasury, expected_held, "Treasury held balance mismatch");
...
fn e2e_vesting_schedule_private2() {
	new_test_ext().execute_with(|| {
		...
		assert!(dust <= 1, "Alice should have full amount at the end. Missing: {dust} raw units");
	});
}
...
fn ensure_no_unexpected_allocations() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(count, 3, "Should have exactly 3 auto-allocations for Treasury");
...
fn ensure_vesting_really_locks_funds_for_users() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(pallet_balances::Pallet::<Runtime>::total_balance(&alice), 950 * AFT);
	});
}
