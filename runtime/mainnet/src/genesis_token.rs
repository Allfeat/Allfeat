...
//! The mainnet tokenomics: 1 billion AFT split into allocation envelopes
//! (upfront rate, cliff and linear vesting per envelope), enforced at
//! genesis by `pallet-token-allocation`.

use crate::{AccountId, Balance, MONTHS, Runtime, Treasury};
use allfeat_runtime_common::currency::UNIT as AFT;
use alloc::vec;
use pallet_token_allocation::{EnvelopeConfig, EnvelopeId};
use polkadot_sdk::{pallet_balances, sp_runtime::Percent};
	...
	TokenGenesis {
		...
		allocations: pallet_token_allocation::GenesisConfig {
			envelopes: vec![
				...
				// contracts (12-month cliff). The value is fixed here so any
				// fresh chain is correct from block 0 and does not
				// reintroduce the bug.
				(
					EnvelopeId::Public2,
					EnvelopeConfig {
