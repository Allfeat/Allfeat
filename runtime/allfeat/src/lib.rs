...
use frame_support::{
	construct_runtime, derive_impl, dispatch::DispatchClass, pallet_prelude::Get, parameter_types, traits::{
		fungible::{Balanced, Credit, HoldConsideration}, ConstU16, ConstU32, EitherOf, EqualPrivilegeOnly, Everything,
		FindAuthor, InstanceFilter, KeyOwnerProofSystem, LinearStoragePrice, OnFinalize, Imbalance, OnUnbalanced,
		WithdrawReasons
	}, weights::{
		constants::{
...
use sp_runtime::{
	...
	ApplyExtrinsicResult, ConsensusEngineId, FixedU128, Perbill, Percent, RuntimeAppPublic,
};
use sp_std::prelude::*;
...
use pallet_evm::{Account as EVMAccount, EVMFungibleAdapter, IdentityAddressMapping, Runner};
#[cfg(any(feature = "std", test))]
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("allfeat-mainnet"),
	impl_name: create_runtime_str!("allfeatlabs-allfeat"),
	authoring_version: 0,
...
impl InstanceFilter<RuntimeCall> for ProxyType {
	fn filter(&self, c: &RuntimeCall) -> bool {
		match self {
			ProxyType::Any => true,
			ProxyType::NonTransfer => !matches!(c, RuntimeCall::Balances(..)), /* TODO fill NonTransfer */
			ProxyType::Staking => {
				matches!(
					c,
					RuntimeCall::Staking(..) |
...
use shared_runtime::{
	...
	BlockHashCount, RuntimeBlockLength, SlowAdjustingFeeUpdate, TransactionPaymentGasPrice,
	U256ToBalance, AVERAGE_ON_INITIALIZE_RATIO, NORMAL_DISPATCH_RATIO,
};
use sp_runtime::{
	traits::{ConvertInto, DispatchInfoOf, PostDispatchInfoOf},
...
parameter_types! {
	pub const MinVestedTransfer: Balance = 1 * AFT;
