...
fn transfer_not_enough_funds() {
	ExtBuilder::default()
		.with_balances(vec![(CryptoAlith.into(), 1000), (CryptoBaltathar.into(), 1000)])
		.build()
		.execute_with(|| {
			precompiles()
				.prepare_test(
		...
		.execute_with(|| {
			precompiles()
				.prepare_test(
					CryptoAlith,
					Precompile1,
					PCall::approve { spender: Address(Bob.into()), value: 300.into() },
				)
				.execute_some();
...
fn withdraw() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// Withdraw
			precompiles()
				.prepare_test(CryptoAlith, Precompile1, PCall::withdraw { value: 500.into() })
				.expect_cost(1381)
				.expect_log(log2(
					Precompile1,
...
fn withdraw_more_than_owned() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// Withdraw
			precompiles()
				.prepare_test(CryptoAlith, Precompile1, PCall::withdraw { value: 1001.into() })
