...
pub struct Eip2612<Runtime, Metadata>(PhantomData<(Runtime, Metadata)>);

impl<Runtime, Metadata> Eip2612<Runtime, Metadata>
where
	...
	<Runtime as pallet_timestamp::Config>::Moment: Into<U256>,
{
	...
	pub(crate) fn permit(
		...
		NoncesStorage::insert(owner, nonce + U256::one());

		{
			let amount = Erc20BalancesPrecompile::<Runtime, Metadata>::u256_to_amount(value)
				.unwrap_or_else(|_| Bounded::max_value());

			let owner: Runtime::AccountId = Runtime::AddressMapping::into_account_id(owner);
			let spender: Runtime::AccountId = Runtime::AddressMapping::into_account_id(spender);
			ApprovesStorage::<Runtime>::insert(owner, spender, amount);
		}
