# Thin Melodie dev-node image: the official polkadot-omni-node image plus a
# dev chain spec generated from the runtime wasm built at the same commit.
# Nothing is compiled in here — see .github/workflows/build-push-dev-image.yml,
# which builds the wasm, generates melodie-dev.json and passes it as context.
#
...
COPY melodie-dev.json /specs/melodie-dev.json
...
CMD ["--chain", "/specs/melodie-dev.json", "--dev-block-time", "6000", "--tmp", "--rpc-external", "--rpc-cors=all"]
